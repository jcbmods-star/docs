...
# --- Configuration ---
TOKEN = '7612716353:AAH7PmZb-PJZjpDDWD1I923bUOUS27rE_Pc'
OWNER_ID = 8422025154
ADMIN_ID = 7974085188
YOUR_USERNAME = '@JCDMOD1'
UPDATE_CHANNEL = "https://t.me/JCDMODS"  # Fixed URL format
...
def install_missing_requirements(req_path, message):
    """Install only missing requirements from requirements.txt"""
    try:
        ...
        if not missing_packages:
            bot.reply_to(message, f"✅ All packages already installed: {', '.join(installed_packages)}")
            return True
        
        # Install only missing packages
        bot.reply_to(message, f"🔄 Installing {len(missing_packages)} missing packages...")
        ...
        for package in missing_packages:
            try:
                ...
                bot.reply_to(message, f"❌ Timeout installing {package}")
                return False
        ...
        bot.reply_to(message, f"✅ Successfully installed {len(missing_packages)} packages")
        return True
...
def attempt_install_pip(module_name, message):
    ...
    try:
        ...
        error_msg = f"❌ Timeout installing `{package_name}`"
        logger.error(error_msg)
        bot.reply_to(message, error_msg)
        return False
    except Exception as e:
    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                ...
                logger.info("Python Pre-check timed out (>10s), imports likely OK. Killing check process.")
...
def remove_subscription_db(user_id):
    with DB_LOCK:
        ...
        try:
            ...
            logger.error(f"❌ SQLite error removing subscription for {user_id}: {e}")
        except Exception as e:
            logger.error(f"❌ Unexpected error removing subscription for {user_id}: {e}", exc_info=True)
        finally:
            conn.close()
...
def add_admin_db(admin_id):
    with DB_LOCK:
        ...
        try:
            ...
            logger.error(f"❌ SQLite error adding admin {admin_id}: {e}")
        except Exception as e:
            logger.error(f"❌ Unexpected error adding admin {admin_id}: {e}", exc_info=True)
        finally:
            conn.close()
...
def remove_admin_db(admin_id):
    ...
    with DB_LOCK:
        ...
        try:
            ...
            logger.error(f"❌ SQLite error removing admin {admin_id}: {e}")
            return False
        except Exception as e:
            logger.error(f"❌ Unexpected error removing admin {admin_id}: {e}", exc_info=True)
            return False
        finally:
            conn.close()
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        logger.info(f"All extracted items: {extracted_items}")
        
        # Find all Python files
        py_files = [f for f in extracted_items if f.endswith('.py')]
        all_files = [f for f in extracted_items if os.path.isfile(os.path.join(temp_dir, f))]
        
        # Handle requirements.txt - Install only missing packages
        req_file = 'requirements.txt' if 'requirements.txt' in extracted_items else None
        if req_file:
            req_path = os.path.join(temp_dir, req_file)
            logger.info(f"requirements.txt found: {req_path}")
            bot.reply_to(message, f"📦 Checking requirements.txt for missing packages...")
            
            # Install only missing requirements
            if install_missing_requirements(req_path, message):
                bot.reply_to(message, "✅ Requirements check completed!")
            else:
                bot.reply_to(message, "⚠️ Some requirements failed to install, but continuing...")

        # Find main script
        main_script_name = None
        file_type = 'py'
        preferred_py = ['main.py', 'bot.py', 'app.py', 'start.py']
        ...
        if not main_script_name:
            bot.reply_to(message, "❌ No `.py` script found in archive!")
            return

        logger.info(f"Main script identified: {main_script_name}")
        logger.info(f"All files to move: {all_files}")
        ...
        # Save ONLY the main script in database (for display in file list)
        save_user_file(user_id, main_script_name, file_type)
        
        # But track all files for this user
        user_folder_contents = os.listdir(user_folder)
        logger.info(f"User folder contents after move: {user_folder_contents}")

        logger.info(f"Saved main script '{main_script_name}' ({file_type}) for {user_id} from zip.")
        main_script_path = os.path.join(user_folder, main_script_name)
        
        bot.reply_to(message, f"✅ All files extracted successfully! Starting main script: `{main_script_name}`...", parse_mode='Markdown')
...
def handle_file_upload_doc(message):
    ...
    if file_ext not in ['.py', '.zip']:
        bot.reply_to(message, "⚠️ Unsupported type! Only `.py`, `.zip` allowed.")
        return
...
def delete_bot_callback(call):
    try:
        ...
        bot.answer_callback_query(call.id, f"🗑️ Deleting {file_name} and all associated files for user {script_owner_id}...")
        script_key = f"{script_owner_id}_{file_name}"
        ...
        # Delete ALL files in user folder (complete cleanup)
        deleted_files = []
        ...
        deleted_str = ", ".join(f"`{f}`" for f in deleted_files[:10])  # Show first 10 files
        if len(deleted_files) > 10:
            deleted_str += f" and {len(deleted_files) - 10} more files"
            
        try:
            bot.edit_message_text(
                f"🗑️ All files for User `{script_owner_id}` deleted!\nDeleted: {deleted_str}",
            ...
            bot.send_message(chat_id_for_reply, f"🗑️ All files for User `{script_owner_id}` deleted.", parse_mode='Markdown')
